...
public class ContractTests {
    ...
    public void tokenContractRequiresZeroInputsInTheTransaction() {
        transaction(ledgerServices, tx -> {
            ...
            tx.input(TokenContract.TOKEN_CONTRACT_ID, tokenState);
            tx.output(TokenContract.TOKEN_CONTRACT_ID, tokenState);
    ...
    @Test
    public void tokenContractAllowsMoreThanOneOutputInTheTransaction() {
        transaction(ledgerServices, tx -> {
    ...
    public void tokenContractRequiresOneCommandInTheTransaction() {
        transaction(ledgerServices, tx -> {
            ...
            tx.command(Arrays.asList(alice.getPublicKey(), bob.getPublicKey()), new TokenContract.Commands.Issue());
            tx.command(Arrays.asList(alice.getPublicKey(), bob.getPublicKey()), new TokenContract.Commands.Move());
            tx.fails();
            return null;
        });
    ...
    public void tokenContractRequiresTheTransactionsOutputToBeATokenState() {
        transaction(ledgerServices, tx -> {
            ...
            tx.output(TokenContract.TOKEN_CONTRACT_ID, new DummyState());
            tx.command(Arrays.asList(alice.getPublicKey(), bob.getPublicKey()), new TokenContract.Commands.Issue());
            tx.fails();
            return null;
        });
    ...
    public void tokenContractRequiresTheTransactionsOutputToHaveAPositiveAmount() {
        ...
        transaction(ledgerServices, tx -> {
            ...
            tx.output(TokenContract.TOKEN_CONTRACT_ID, zeroTokenState);
            tx.command(Arrays.asList(alice.getPublicKey(), bob.getPublicKey()), new TokenContract.Commands.Issue());
            tx.fails();
            return null;
        });
        ...
        transaction(ledgerServices, tx -> {
            ...
            tx.output(TokenContract.TOKEN_CONTRACT_ID, negativeTokenState);
            tx.command(Arrays.asList(alice.getPublicKey(), bob.getPublicKey()), new TokenContract.Commands.Issue());
            tx.fails();
            return null;
        });
        ...
        transaction(ledgerServices, tx -> {
            ...
            tx.output(TokenContract.TOKEN_CONTRACT_ID, positiveTokenState);
            tx.command(Arrays.asList(alice.getPublicKey(), bob.getPublicKey()), new TokenContract.Commands.Issue());
            tx.verifies();
            return null;
        });
    ...
    public void tokenContractRequiresTheIssuerToBeARequiredSignerInTheTransaction() {
        ...
        transaction(ledgerServices, tx -> {
            ...
            tx.output(TokenContract.TOKEN_CONTRACT_ID, tokenStateWhereBobIsIssuer);
